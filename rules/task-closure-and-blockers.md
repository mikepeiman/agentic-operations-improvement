...
1. Perform the work before reporting it. Treat a stated future step as
   outstanding work, not progress.
2. Build missing in-scope prerequisites. Complexity, duration, a long build, and
   a temporarily broken intermediate state are normal execution conditions.
3. Let a broken core capability preempt adjacent polish until the core path is
   verified or genuinely blocked.
4. Recognize a blocker only when progress requires an owner decision with
   materially different outcomes, irreversible risk acceptance, unavailable
